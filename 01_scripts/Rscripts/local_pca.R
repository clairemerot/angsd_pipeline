...
info_pc<-read.table(paste0("04_pca/all_maf",MIN_MAF,"_pctind",PERCENT_IND,".pca"))

#import the list of covariance matrix by window
window<-read.table(paste0("10_pca_by_window/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"/",WINDOW_SIZE,"/analyse_by_window/list_window.txt"), header=F)
...
for (j in 1:length(LGs))
{
	...
	for (i in 1: dim(window_order_j)[1])
	{
		...
		window_covmats[[i]]<-as.matrix(read.table(paste0("10_pca_by_window/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"/",WINDOW_SIZE,"/cov_by_window/",window_order_j$file_name[i])))
	...
	if(length(window_covmats)>1)
	{
		...
		jpeg(paste0("10_pca_by_window/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"/",WINDOW_SIZE,"/analyse_by_window/mds_",LGs[j],".jpg"))
		...
		jpeg(paste0("10_pca_by_window/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"/",WINDOW_SIZE,"/analyse_by_window/mds_bypos_h_",LGs[j],".jpg"), height=1000, width=600)
		par(mfrow=c(3,2))
		...
		if (n>0)
		{
			...
			jpeg(paste0("10_pca_by_window/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"/",WINDOW_SIZE,"/analyse_by_window/corr_with_info_",LGs[j],".jpg"), height=1200, width=600)
			par(mfrow=c(n/2,2))
			for (k in 1 : n)
			{
...
write.table(as.matrix(window_result_bis[2:(dim(window_result_bis)[1]),]),paste0("10_pca_by_window/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"/",WINDOW_SIZE,"/analyse_by_window/window_result.txt"), quote=F,row.names=F)  
